...
(function(){
 ...
 function finish(){saveFoundation('place');saveFoundation('finger');const p=profile();const app=document.querySelector('#app');app.innerHTML=`<div class="screen world-screen"><header class="topbar"><button class="icon-btn" id="bjFinishBack" aria-label="Go back">←</button><div class="brand"><span class="brand-mark">🧮</span><span><strong>Abacus AI</strong><small>My first adventure</small></span></div></header><main class="content"><div style="max-width:620px;margin:auto;text-align:center">${babi()}<p class="eyebrow">ADVENTURE COMPLETE</p><h1>Look at you, ${String(p.name||'abacus star').replace(/[<>]/g,'')}! 🌟</h1><p style="font-size:20px;line-height:1.5">You met the abacus, learned 1 and 5, and built numbers with your own hands.</p><div class="learning-ready"><h2>🎯 You’re ready!</h2><p>Now Babi will give you real Level 1 practice.</p><button class="primary" id="bjStart">Start Level 1 🚀</button></div></div></main></div>`;document.querySelector('#bjFinishBack').onclick=()=>{step=steps.length-1;render()};document.querySelector('#bjStart').onclick=()=>{location.hash='start-level-1';location.reload()}}
