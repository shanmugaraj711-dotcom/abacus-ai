...
(function(){
  ...
  async function startApp(){try{await import('./challengeAppV2.js');if('requestIdleCallback' in window)requestIdleCallback(loadEnhancements,{timeout:2500});else setTimeout(loadEnhancements,1200)}catch(err){console.error(err);fail()}}
  const profile=read();
  if(profile){app.innerHTML='<div class="screen onboarding center"><div style="margin:auto"><div style="font-size:52px">🧮</div><h1>Babi is waking up…</h1></div></div>';startApp()}else{
    ...
    next.onclick=async()=>{if(busy||next.disabled)return;busy=true;next.disabled=true;const p={name:name.value.trim(),age,experience:exp,createdAt:Date.now()};try{localStorage.setItem(PROFILE,JSON.stringify(p))}catch{busy=false;next.disabled=false;return}if(exp==='known'){await import('./challengeAppV2.js').catch(fail);return}startApp()};
  }
  window.addEventListener('load',()=>{if('serviceWorker' in navigator)navigator.serviceWorker.register('./sw.js?v=20260915-v31').catch(()=>{})});
})();
