...
(function(){
  ...
  function clearVisualDemo(card){
    all('.lp-demo-active',card).forEach(el=>el.classList.remove('lp-demo-active'));
    one('.lesson-demo-display',card)?.remove();
  }
  ...
  function renderState(card,state,target){
    ...
    if(state===STATES.IDLE){
      setInput(card,false); if(check)check.disabled=true; if(next)next.disabled=true;
      ...
      if(feedback)feedback.textContent=`Watch: Babi is building ${target}, one move at a time.`;
    } else if(state===STATES.PRACTICE){
      setInput(card,true); if(check)check.disabled=false; if(next)next.disabled=true;
      if(show){show.disabled=false;show.textContent='👀 Watch again';}
      if(feedback)feedback.textContent=`Now you try: Make ${target}.`;
    } else if(state===STATES.COMPLETE){
      setInput(card,false); if(check)check.disabled=true; if(next)next.disabled=false;
      if(show){show.disabled=false;show.textContent='✓ Demo complete';}
      if(feedback)feedback.textContent='You got 3/3! 🌟 Next lesson is ready.';
    }
  ...
  function visualValue(card,count){
    ...
    lowers.forEach((bead,i)=>bead.classList.toggle('lp-demo-active',i<count));
    if(upper)upper.classList.toggle('lp-demo-active',count>=5);
    let display=one('.lesson-demo-display',card);
    if(!display){display=document.createElement('div');display.className='lesson-demo-display';display.innerHTML='<span>Babi shows</span><strong>0</strong>';wrap.insertAdjacentElement('afterend',display)}
    one('strong',display).textContent=String(count);
  }
  function playDemo(card,target,finish){
    let cancelled=false; const timers=[];
    cancelDemo=()=>{cancelled=true;timers.forEach(clearTimeout);clearVisualDemo(card)};
    const values=[]; if(target>=5)values.push(5); const remainder=target%5; if(remainder)for(let i=1;i<=remainder;i++)values.push(5+i); if(!values.length)values.push(target);
    visualValue(card,0);
    values.forEach((value,index)=>timers.push(setTimeout(()=>{if(!cancelled)visualValue(card,value)},index*420)));
    timers.push(setTimeout(()=>{if(cancelled)return;clearVisualDemo(card);finish()},values.length*420+500));
  }
  function transition(card,nextState,target){
    renderState(card,nextState,target);
    if(nextState===STATES.DEMO)playDemo(card,target,()=>{
      if(realValue(card)!==0){console.error('[lesson-state] DEMO mutated real answer state');transition(card,STATES.IDLE,target);return}
      transition(card,STATES.PRACTICE,target);
    });
  }
  function install(card){
    if(!card||card===activeCard)return;
    activeCard=card; cancelDemo();
    const target=targetOf(card); if(!Number.isFinite(target))return;
    one('.lp-coach',card)?.remove(); one('.lp-show',card)?.remove();
    const check=one('#lessonCheck',card); if(!check)return;
    const show=document.createElement('button');show.type='button';show.id='lessonWatch';show.className='lp-show';check.insertAdjacentElement('afterend',show);
    transition(card,STATES.IDLE,target);
    show.addEventListener('click',()=>{
      if(card.dataset.lessonState===STATES.IDLE||card.dataset.lessonState===STATES.PRACTICE)transition(card,STATES.DEMO,target);
    });
    check.addEventListener('click',()=>{
      if(card.dataset.lessonState!==STATES.PRACTICE)return;
      setTimeout(()=>{if(realValue(card)===target&&one('#lessonFeedback',card)?.textContent.includes('3/3'))transition(card,STATES.COMPLETE,target)},0);
    });
  }
